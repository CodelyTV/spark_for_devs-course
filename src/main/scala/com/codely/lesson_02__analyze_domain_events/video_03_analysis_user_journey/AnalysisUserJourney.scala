...
object AnalysisUserJourney extends App {
  ...
  val productViewedDF = readJson(
    "src/main/scala/com/codely/lesson_02__analyze_domain_events/video_03_analysis_user_journey/data/productViewed.json"
  )

  val productAddedDF = readJson(
    "src/main/scala/com/codely/lesson_02__analyze_domain_events/video_03_analysis_user_journey/data/addedToCart.json"
  )
  ...
  val userJourneyDF = allEventsDF
    ...
    .agg(
      sort_array(
        collect_list(struct("timestamp", "eventType", "productId")),
        asc = true
      ).as("UserJourney")
    )
    .orderBy("userId", "date")
